...
const char WIFI_SSID[] = "CMB STEAMlab";
const char WIFI_PSK[] = "CMBGuest0987";
const encryption WIFI_EE = WPA2_AES;
const char stationID[] = "MonTerra1"; // ID of the monitoring station, this value will be sent to phant

//const char public_key[] = "YG00rLpOYXFm42n84Kdr";
//const char private_key[] = "RbqqZDE0PwH1pm4KpAvD";
//const char host[] = "data.sparkfun.com";
//const char destIP[] = "54.86.132.254"; 
//const char destPort[] = "50"; // hex for port 80

const char content_format[] = "moisture=%ld&stationid=%s&attempt=%u&sequence=%u";
const char request_format[] = "POST /input/%s.txt HTTP/1.1\r\nHost: %s\r\nPhant-Private-Key: %s\r\nConnection: close\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: %u\r\n\r\n%s";

const char public_key[] = "mqBpa2Gdg6urqo4XzA6rCGvMeMEJ";
const char private_key[] = "jLNzWvK07VhDlaeb9x5DCD0jQjmL";
const char host[] = "69.145.204.62";
const char destIP[] = "69.145.204.62";
const char destPort[] = "1F90"; // hex for port 8080

//Phant phant("data.sparkfun.com", "YG00rLpOYXFm42n84Kdr", "RbqqZDE0PwH1pm4KpAvD");
...
const char moistureField[] = "moisture";
const char stationIDField[] = "stationid";
const char attemptField[] = "attempt";
const char sequenceField[] = "sequence";
...
void loop() {
  ...
  XB_SERIAL.print("POST /input/YG00rLpOYXFm42n84Kdr.txt HTTP/1.1\r\nHost: data.sparkfun.com\r\nPhant-Private-Key: RbqqZDE0PwH1pm4KpAvD\r\nConnection: close\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 54\r\n\r\nmoisture=1234&stationid=MonTerra1&attempt=2&sequence=1");
  Serial.println("POST /input/YG00rLpOYXFm42n84Kdr.txt HTTP/1.1\r\nHost: data.sparkfun.com\r\nPhant-Private-Key: RbqqZDE0PwH1pm4KpAvD\r\nConnection: close\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 54\r\n\r\nmoisture=1234&stationid=MonTerra1&attempt=2&sequence=1");
...
bool sendData(int attempt) {
  ...
  const int MAX_REQUEST_SIZE = 500;
  const int MAX_CONTENT_SIZE = 200;
  char request_buffer[MAX_REQUEST_SIZE];
  char content[MAX_CONTENT_SIZE];

  snprintf(content, MAX_CONTENT_SIZE, content_format, moistureReading, stationID, attempt, seq);
  unsigned int content_length = strlen(content);
  
  snprintf(request_buffer, MAX_REQUEST_SIZE, request_format, public_key, host, private_key, content_length, content);
  ...
  for(int i = 0; i < content_length; i++) {
    Serial.print((int)request_buffer[i], DEC);
    Serial.print(" ");
  }
  Serial.println();
...
void setupHTTP() {
  ...
  char ip_address[20];
  snprintf(ip_address, 20, "ATDL%s", destIP);

  command(ip_address); // set ip address for our requests
  if(!waitForOK()) {
  }

  char port[10];
  snprintf(port, 10, "ATDE%s", destPort);
  command(port); // set port for our requests
  if(!waitForOK()) {
  }
}
...
void connectToWiFi() {
  ...
  char atid[100];
  snprintf(atid, 100, "ATID%s", WIFI_SSID);

  command(atid); // set network SSID
  if(!waitForOK()) {
  }

  char atee[10];
  snprintf(atee, 10, "ATEE%d", (byte)WIFI_EE);

  command(atee); // set network encryption
  if(!waitForOK()) {
  }
  if(WIFI_EE != NO_SECURITY) {
      char atpk[100];
      snprintf(atpk, 100, "ATPK%s", WIFI_PSK);
      command(atpk); // set network password, no commas allowed in password as it's a limitation of the XBee command mode (commas can separate multiple commands)
      if(!waitForOK()) {
      }
  }
